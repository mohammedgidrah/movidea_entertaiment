    ...
    jQuery('.widget_calendar').on('click', '.month_prev a, .month_next a', function(e) {
        ...
        jQuery.post(MELODYSCHOOL_STORAGE['ajax_url'], {
            ...
            try {
                ...
                rez = {
                    error: MELODYSCHOOL_STORAGE['ajax_error']
                };
        ...
        if (!exists) {
            var message_popup = melodyschool_message_dialog('<label for="bookmark_title">' + MELODYSCHOOL_STORAGE['strings']['bookmark_title'] + '</label><br><input type="text" id="bookmark_title" name="bookmark_title" value="' + title + '">', MELODYSCHOOL_STORAGE['strings']['bookmark_add'], null,
                function(btn, popup) {
                    ...
                    setTimeout(function() {
                        melodyschool_message_success(MELODYSCHOOL_STORAGE['strings']['bookmark_added'], MELODYSCHOOL_STORAGE['strings']['bookmark_add']);
...
function melodyschool_prepare_menus() {
    ...
    for (var m in menus) {
        ...
        menus[m].find('li').removeClass('current-menu-ancestor current-menu-parent current-menu-item');
...
function melodyschool_init_media_elements(cont) {
    if (MELODYSCHOOL_STORAGE['media_elements_enabled'] && cont.find('audio,video').length > 0) {
        if (window.mejs) {
            ...
            setTimeout(function() {
                melodyschool_init_media_elements(cont);
            }, 400);
    }
}
...
function melodyschool_login_validate(form) {
    ...
    if (!error) {
        jQuery.post(MELODYSCHOOL_STORAGE['ajax_url'], {
            ...
            if (rez.error === '') {
                result_box.addClass('sc_infobox sc_infobox_style_success').html(MELODYSCHOOL_STORAGE['strings']['login_success']);
                setTimeout(function() {
                    location.reload();
                }, 3000);
            } else {
                result_box.addClass('sc_infobox sc_infobox_style_error').html(MELODYSCHOOL_STORAGE['strings']['login_failed'] + '<br>' + rez.error);
            }
...
function melodyschool_registration_validate(form) {
        ...
        rules: [{
            ...
            },
            {
                ...
                equal_to: {
                    ...
                    message: MELODYSCHOOL_STORAGE['strings']['password_not_equal']
                }
            }
        ]
    });
    if (!error) {
        jQuery.post(MELODYSCHOOL_STORAGE['ajax_url'], {
            ...
            result_box.toggleClass('sc_infobox_style_error', false).toggleClass('sc_infobox_style_success', false);
            if (rez.error === '') {
                result_box.addClass('sc_infobox sc_infobox_style_success').html(MELODYSCHOOL_STORAGE['strings']['registration_success']);
                setTimeout(function() {
                    jQuery('.popup_login_link').trigger('click');
                }, 3000);
            } else {
                result_box.addClass('sc_infobox sc_infobox_style_error').html(MELODYSCHOOL_STORAGE['strings']['registration_failed'] + ' ' + rez.error);
            }
            result_box.fadeIn().delay(3000).fadeOut();
        });
    }
